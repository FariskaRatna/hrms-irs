...
frappe.ui.form.on("Saving", {
	refresh(frm) {
        if (!frm.is_new() && frm.doc.docstatus === 0 && frm.perm[0] == 1) {
            frm.page.set_primary_action(__("Submit"), function() {
                frappe.confirm(
                    __("Permanently submit Saving for {0}?", [frm.doc.employee_name || frm.doc.employee]),
                    function () {
                        frappe.call({
                            ...
                            callback: function (r) {
                                if (!r.exc) {
                                    frappe.show_alert({ message: __("Saving submitted"), indicator: "green" });
                                    frm.reload_doc();
                                }
                            }
                        });
