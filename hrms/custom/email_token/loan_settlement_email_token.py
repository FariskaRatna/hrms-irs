...
def _make_token(loan_name: str, action: str, approver_user: str, hours_valid: int = 72) -> str:
    ...
    doc = frappe.get_doc({
        "doctype": "Loan Settlement Email Token",
        "token": token,
        "reference_doctype": "Loan Settlement",
...
def build_action_url_loan_set(loan_name: str, action: str, approver_user: str) -> str:
    token = _make_token(loan_name, action, approver_user)

    return get_url(
        f"/api/method/hrms.custom.email_token.loan_settlement_email_token.act?name={loan_name}&token={token}"
    )
...
def act(name: str, token: str):
    ...
    loan = frappe.get_doc("Loan Settlement", name)

    current = (loan.get("approval_status") or "").strip()
    if current in ("Approved", "Rejected"):
        frappe.throw(_("Loan Settlement is already {0}").format(frappe.bold(current)), frappe.PermissionError)
    ...
    frappe.db.sql(
        """
        UPDATE `tabLoan Settlement Email Token`
        SET used = 1, used_on = %s
        WHERE reference_doctype = %s
          AND reference_name = %s
          AND used = 0
        """,
        (now_datetime(), "Loan Settlement", name),
    )
    ...
    frappe.local.response["location"] = get_url(
        f"/message?title=Success&message=Loan%20Settlement%20{action}%20saved."
    )
