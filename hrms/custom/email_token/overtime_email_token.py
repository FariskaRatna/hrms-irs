...
def _make_token(bt_name: str, action: str, approver_user: str, hours_valid: int = 72) -> str:
    ...
    doc = frappe.get_doc({
        "doctype": "Overtime Email Token",
        "token": token,
        "reference_doctype": "Overtime",
...
def build_action_url(bt_name: str, action: str, approver_user: str) -> str:
    ...
    return get_url(
        f"/api/method/hrms.custom.email_token.overtime_email_token.act?name={bt_name}&token={token}"
    )

@frappe.whitelist(allow_guest=True)
def act(name: str, token: str):
    rows = frappe.get_all(
        "Overtime Email Token",
        filters={
            "token": token,
            "reference_doctype": "Overtime",
            "reference_name": name
    ...
    bt = frappe.get_doc("Overtime", name)
    ...
    frappe.db.sql(
        """
        UPDATE `tabOvertime Email Token`
        SET used = 1, used_on = %s
        WHERE reference_doctype = %s
          AND reference_name = %s
          AND used = 0
        """,
        (now_datetime(), "Overtime", name),
    )
    ...
    frappe.local.response["location"] = get_url(
        f"/message?title=Success&message=Overtime%20{action}%20saved."
    )