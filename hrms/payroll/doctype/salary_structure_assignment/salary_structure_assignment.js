	...
	refresh: function (frm) {
		...
		if (!frm.is_new() && frm.doc.docstatus === 0) {
			frm.page.set_primary_action(__("Submit"), function() {
				frappe.confirm(
					`Permanently submit Salary Structure Assignment for ${frm.doc.employee_name || frm.doc.employee}?`,
					function() {
						frappe.call({
							...
							callback: function (r) {
								if (!r.exc) {
									frappe.show_alert({ message: __("Salary Structure Assignment submitted"), indicator: "green" });
									frm.reload_doc();
								}
							}
						});
					...
					function () {
						frappe.show_alert({ message: __("Salary Structure Assignment cancelled"), indicator: "red" });
					}
				);
			});
		}
